...
| R5 | Native verification | PASS WITH CONCERNS | One ARM64 and one x64 runner verify complete trust and smoke facts; fail-closed Rosetta detection and aggregation negatives pass. Real hosted native documents are deferred. |
...
The repository proves orchestration and fail-closed contracts, not the live
Apple or protected-environment facts. Merge is appropriate for this sequential
slice only if the tracker continues to show those requirements as `NOT YET`
and P9 consumes a fresh unused RC.
