...
| Dependency audit | PASS | 0 vulnerabilities in the new CLI runtime dependency; the complete portable audit passed. |
...
| R6 - provenance and containment | PASS | The authoritative JSON root binds the exact HTML, both typed stage outputs, both stage receipts, and the generated-artifact validation receipt. Relative/external resources, CSS imports, external script sources, forbidden embedded elements, runtime errors, and broken links fail before passage; Desktop adds a no-authority CSP sandbox. |
...
## Adversarial checks added after attempt 10

- A relative `<script src="helper.js">` and CSS `@import` both fail containment.
- An inline JavaScript exception fails generated-artifact validation.
- Missing native reveal summaries and inaccessible visual markup fail generated-artifact validation.
- A stale or substituted generated-artifact receipt fails digest validation.
- A receipt whose requested capability profile differs from the module request leaves the workflow `UNSET`.
