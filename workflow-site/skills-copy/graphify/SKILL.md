...
Three things it does that a single-session agent context cannot:
...
This step has two parts: **structural extraction** (deterministic, free) and **semantic extraction** (LLM-based, costs tokens).
...
- Images (.png/.jpg/.webp) → downloaded, vision-capable model extracts on next run
...
## For native agent-instruction integration

Run once per project to make graphify always-on in agent sessions:

```bash
graphify claude install
```

This writes a `## graphify` section to the local agent instruction file, such as `AGENTS.md` or `CLAUDE.md`, that instructs agents to check the graph before answering codebase questions and rebuild it after code changes. No manual `/graphify` needed in future sessions.
