...
Use this as a gate, not a checklist. Review the formal boundary's pinned diff,
or the current Git repository's branch diff for a standalone invocation,
against active pattern rules.

Deterministic helper:

```bash
python3 "<plugin-root>/resources/scripts/pattern_tool.py" review-inputs --cwd <repo>
```

At a formal PR boundary, first load the persisted PR context through
`boundary_gate.py --gate patternReview`. Pass its `diffBaseSha` and
`diffHeadSha` as `--base` and `--head` to the helper, and write only to the
returned packet `outputPath`. Formal gates never infer their own Git context.
...
10. Write the final human-readable report before declaring the gate passed.
   Formal boundaries use the resolved packet output; standalone work uses an
   explicitly named or legacy cumulative-feature report.
